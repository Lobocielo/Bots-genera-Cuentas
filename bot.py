...
# --- API Core Logic (1:1 with Original) ---

def generate_random_name(total_length=12, custom_prefix=None, region=None):
    superscript_digits = ['⁰', '¹', '²', '³', '⁴', '⁵', '⁶', '⁷', '⁸', '⁹']
    base_name = custom_prefix if custom_prefix else "1shot"
...
def generate_custom_password(random_length=9):
    ...
    return f"1shot-{random_part}-X64"
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"{Fore.RED}[!] Register failed: {response.status_code}")
            time.sleep(1)
        except Exception as e: 
            print(f"{Fore.RED}[!] Exception in Register: {e}")
            continue
    ...
    try:
        ...
        if resp.status_code == 200:
            if lang.lower() not in ["ar", "en"]:
                json_result = get_available_room(resp.content.hex())
                parsed = json.loads(json_result)
                jwt_token = parsed['8']['data']
                fields = bytes.fromhex(encrypt_api(CrEaTe_ProTo({1: "RU" if region.lower() == "cis" else region}).hex()))
                ...
                if r_cr.status_code == 200: return login_server(uid, password, access_token, open_id, response, status_code, name, region)
            
            jwt_token = resp.text[resp.text.find("eyJhbGciOiJIUzI1NiIsInN2ciIj"):-1]
            if not jwt_token: jwt_token = resp.text[resp.text.find("eyJhbGciOiJIUzI1NiIsInN2ciI6I"):-1]
            dot_idx = jwt_token.find(".", jwt_token.find(".") + 1)
            jwt_token = jwt_token[:dot_idx+44]
            return GET_PAYLOAD_BY_DATA(jwt_token, access_token, 1, region, None, uid, password)
...
def login_server(uid, password, access_token, open_id, response, status_code, name, region):
    try:
        ...
        resp = requests.post(url, headers=headers, data=body, verify=False, timeout=30)
        if resp.status_code == 200:
            jwt_token = json.loads(get_available_room(resp.content.hex()))['8']['data']
            dot_idx = jwt_token.find(".", jwt_token.find(".") + 1)
            jwt_token = jwt_token[:dot_idx+44]
            return GET_PAYLOAD_BY_DATA(jwt_token, access_token, 1, region, None, uid, password)
...
def GET_PAYLOAD_BY_DATA(JWT_TOKEN, NEW_ACCESS_TOKEN, date, region, custom_prefix=None, original_uid=None, original_password=None):
    try:
        ...
        PAYLOAD = PAYLOAD.replace(b"2023-12-24 04:21:34", now.encode()).replace(b"15f5ba1de5234a2e73cc65b6f34ce4b299db1af616dd1dd8a6f31b147230e5b6", NEW_ACCESS_TOKEN.encode("UTF-8")).replace(b"4666ecda0003f1809655a7a8698573d0", NEW_EXTERNAL_ID.encode("UTF-8")).replace(b"7428b253defc164018c604a1ebbfebdf", SIGNATURE_MD5.encode("UTF-8"))
...
def GET_LOGIN_DATA(JWT_TOKEN, PAYLOAD, region):
    ...
    headers = {'Authorization': f'Bearer {JWT_TOKEN}', 'X-Unity-Version': '2018.4.11f1', 'X-GA': 'v1 1', 'ReleaseVersion': 'OB52', 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'Dalvik/2.1.0 (Linux; U; Android 10; G011A Build/PI)', 'Host': 'clientbp.ggblueshark.com', 'Connection': 'close'}
    try:
...
def is_paid(user_id):
    ...
    db = load_db() ; user_str = str(user_id)
...
@bot.tree.command(name="info", description="Ver perfil")
...
@bot.tree.command(name="announce", description="📢 Enviar anuncio")
async def announce(interaction: discord.Interaction):
    await interaction.response.defer()
    if interaction.user.id != OWNER_ID:
        await interaction.followup.send("❌ No autorizado.", ephemeral=True) ; return
    await interaction.channel.send(content="@everyone @here", embed=discord.Embed(title="📢 ZENIHT AVISO", description="Bot activo. Genera tus cuentas ahora.", color=0xFF4500))
